...
exports.handler = async (event) => {
  ...
  try {
    ...
    const { monthKey, pattern, draining, stabilizing, direction, lesson } = JSON.parse(event.body);

    if (!monthKey) {
      return {
        statusCode: 400,
        body: JSON.stringify({ error: 'Month key is required' })
      };
    }

    const store = getStore('reflection-entries');
    const key = `${user.userId}:monthly:${monthKey}`;
